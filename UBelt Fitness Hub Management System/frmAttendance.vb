...
#Region "Daily"
    ...
    Public Sub load_MembersInfoDaily()
        ...
        & ", `Date` AS 'Date', `TimeLogIn` AS 'Log In', `TimeLogOut` as 'Log Out' FROM `daily`"
    ...
    Private Sub txtDailySearchBox_TextChanged(sender As Object, e As EventArgs) Handles txtDailySearchBox.TextChanged
        query = "SELECT `MemberId` AS 'Member ID', `FullName` AS 'Full Name'" _
        & ", `Date` AS `Date`, `TimeLogIn` AS 'Log In', `TimeLogOut` AS 'Log Out'" _
    ...
    Private Sub btnDTimeIn_Click(sender As Object, e As EventArgs) Handles btnDTimeIn.Click
        Try
            Dim logdate As String = String.Format("{0:MMMMM dd, yyyy}", Date.Now)
        Dim login = TimeOfDay
            query = "INSERT INTO `daily` (`MemberId`, `FullName`, `MembershipType`, `Date`, `TimeLogIn`)" _
    ...
    Private Sub DeleteToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles DeleteToolStripMenuItem.Click
        query = "DELETE FROM daily WHERE TimeLogIn = '" & dgvDailyMembersRecord.CurrentRow.Cells(3).Value & "'"
        deletes(query, dgvDailyMembersRecord.CurrentRow.Cells(1).Value)
        load_MembersInfoDaily()
    End Sub
    Private Sub dgvDailyMembersRecord_CellToolTipTextNeeded(sender As Object, e As DataGridViewCellToolTipTextNeededEventArgs) Handles dgvDailyMembersRecord.CellToolTipTextNeeded
...
#Region "Monthly"
    ...
    Public Sub load_MembersInfoMonthly()
        query = "SELECT `MemberId` AS 'Member Id', `FullName` AS 'Full Name'" _
        & ", `Date` AS 'Date', `TimeLogIn` AS 'Log In', `TimeLogOut` as 'Log Out' FROM `monthly`"
    ...
    Private Sub btnMTimeIn_Click(sender As Object, e As EventArgs) Handles btnMTimeIn.Click
        Try
            Dim logdate As String = String.Format("{0:MMMMM dd, yyyy}", Date.Now)
            Dim login = TimeOfDay
            query = "INSERT INTO `monthly` (`MemberId`, `FullName`, `MembershipType`, `Date`, `TimeLogIn`)" _
    ...
    Private Sub ToolStripMenuItem1_Click(sender As Object, e As EventArgs) Handles ToolStripMenuItem1.Click
        query = "DELETE FROM monthly WHERE TimeLogIn = '" & dgvMonthlyMembersRecord.CurrentRow.Cells(3).Value & "'"
